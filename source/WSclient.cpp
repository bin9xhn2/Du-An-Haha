...
#include "Reconnect.h"	//add Reconnect
#include "HealthBar.h"
...
{
	g_pReconnect->ReconnectCreateConnection(IpAddr, Port);  //Add Reconnect
	BOOL bResult = TRUE;
...
{
	InitGuildWar();
	g_pReconnect->ReconnectOnCharacterList();  //Add Reconnect
...
BOOL ReceiveLogOut(BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	LogOut = false;
	LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
	g_pReconnect->ReconnectOnCloseClient(Data->Value);  //Add Reconnect
	switch (Data->Value)
	{
	case 0:
...
{
	g_pReconnect->ReconnectOnCharacterInfo();  //Add Reconnect
	MouseLButton = false;
...
void ReceiveChangeMapServerInfo(BYTE* ReceiveBuffer)
{
	...
	g_pReconnect->ReconnectOnMapServerMove(Data->m_vSvrInfo.m_szMapSvrIpAddress, Data->m_vSvrInfo.m_wMapSvrPort);  //Add Reconnect
...
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	g_pReconnect->ReconnectOnMapServerMoveAuth(Data->Value);  //Add Reconnect
...
BOOL TranslateProtocol(int HeadCode, BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case 0xF1:
	{
		...
		case 0x01: //receive log in
			//AddDebugText(ReceiveBuffer,Size);
			g_pReconnect->ReconnectOnConnectAccount(Data->Value);  //Add Reconnect
			switch (Data->Value)
			{
			case 0x20:
