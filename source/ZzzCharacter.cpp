...
void RenderGuildAndNameAlways(CHARACTER* c);
...
#endif// GUILD_WAR_EVENT
        {
			for(int i=MAX_BODYPART-1; i>=0; i--)
			{
				...
				if(p->Type != -1)
				{
					...
					if (o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && g_pOption->m_Equipments && i > 0)															  //AntLag + Borderless
					{																																					  //AntLag + Borderless
						int COMMON_NUM = ((i - 1) * MODEL_BODY_NUM) + MODEL_BODY_HELM;																					  //AntLag + Borderless
						Type = gCharacterManager.GetSkinModelIndex(c->Class) + COMMON_NUM;																				  //AntLag + Borderless
						RenderPartObject(&c->Object, Type, p, c->Light, o->Alpha, 0 << 3, false, false, false, false, Translate, Select);								  //AntLag + Borderless
					}																																					  //AntLag + Borderless
					else																																				  //AntLag + Borderless
					{																																					  //AntLag + Borderless
						RenderPartObject(&c->Object, Type, p, c->Light, o->Alpha, p->Level << 3, p->Option1, p->ExtOption, false, false, Translate, Select);			  //AntLag + Borderless
					}																																					  //AntLag + Borderless
...
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
			if (g_pOption->m_Swords == 0)																					 //AntLag + Borderless
					{																										 //AntLag + Borderless
						RenderLinkObject(0.f, 0.f, 0.f, c, w, w->Type, w->Level, w->Option1, false, Translate);				 //AntLag + Borderless
					}																										 //AntLag + Borderless
				if(w->Level >= 7)
				{
		...
		if (o->Live)
		{
			if (o->Visible)
			{
				if (i != SelectedCharacter && i != SelectedNpc)
					RenderCharacter(c, o);
				//if (o->Type == MODEL_PLAYER && o->Live)										 //Khó quá thì show logo guide bằng cách này
				//{																			 //Khó quá thì show logo guide bằng cách này
				//	CreateChat(c->ID, "", c); // ép render chat rỗng						 //Khó quá thì show logo guide bằng cách này
				//}																			 //Khó quá thì show logo guide bằng cách này
				else
...
// 			}
			...
			if(bBack && iType != -1)
			{
				...
				{																																	  //AntLag + Borderless
					bool bRightHandItem = false;																									  //AntLag + Borderless
					if (i == 0)																														  //AntLag + Borderless
						bRightHandItem = true;																										  //AntLag + Borderless
					if (g_pOption->m_Swords == 0)																									  //AntLag + Borderless
					{																																  //AntLag + Borderless
						RenderLinkObject(0.f, 0.f, 15.f, c, &t_Part, iType, iLevel, iOption1, true, bTranslate, 0, bRightHandItem);					  //AntLag + Borderless
					}																																  //AntLag + Borderless
				}																																	  //AntLag + Borderless
				w->AnimationFrame = fAnimationFrameBackUp;																							  //AntLag + Borderless
				/*else																																 //Zin
				{																																	 //Zin
					bool bRightHandItem = false;																									 //Zin
					if(i == 0)																														 //Zin
						bRightHandItem = true;																										 //Zin
					RenderLinkObject(0.f,0.f,15.f,c,&t_Part,iType,iLevel,iOption1, true, bTranslate, 0, bRightHandItem);							 //Zin
				}																																	 //Zin
																																					 //Zin
				w->AnimationFrame = fAnimationFrameBackUp;*/																						 //Zin
			}
        ...
        if(gMapManager.InChaosCastle() == false)
        {
			...
			PART_t *w = &c->Wing;
			if (w->Type != -1 && g_pOption->m_Wings == 0);								   // AntLag + Borderless
			if(w->Type != -1)
			{
				w->LinkBone = 47;
				if(o->CurrentAction == PLAYER_FLY||o->CurrentAction == PLAYER_FLY_CROSSBOW)
